...
Completion review found that the Android ACTIVE gap tracker used by these formal runs omitted the interval entering a window and the tail from the last frame to the one-second window end. The implementation now measures both boundaries, and the feature-stage runner now requires an eligible winner authorized by the base `report.json`. The historical 493.7 ms B0 gap is therefore inconclusive rather than a proven freeze pass; the early-stop decision remains independently supported by B0's static-image-quality and content-aware failures.
