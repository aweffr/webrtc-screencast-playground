...
- 2026-07-18: Raw VideoToolbox and actual RTC encoder probes produced identity software-decoded patch values for all H.264/HEVC × `420f`/`420v` cells on Apple AVE hardware. The encoder and RTC wrapper are not the first divergent boundary.
- 2026-07-18: Direct Android Surface decode reproduced codec-specific FULL behavior: the H.264 decoder normalized full-range input to video-range Surface values while the HEVC decoder retained full-range values. Both decoders produced the same Surface values for `420v`. Candidate A was therefore selected without combining encoder or renderer workarounds.
- 2026-07-18: TDD changed ScreenCaptureKit to `420v` and pinned SDR Rec.709 matrix/color space. Full downstream verification passed, the Android `420v` instrumentation contract passed, and builder tests reached 114/114.
- 2026-07-18: The bounded A0/A1 E2E completed 2/2 valid with zero retry and content-aware MaxQP 24/32 applied. HEVC worst static SSIM-Y/PSNR-Y improved from 0.9758/35.54 dB to 0.99774/40.08 dB versus the concurrent H.264 reference at 0.99720/38.25 dB. Original-resolution manual inspection found no remaining codec-specific brightness lift or clipping. The result is documented in `docs/experiments/2026-07-18-hevc-color-range.md`.
