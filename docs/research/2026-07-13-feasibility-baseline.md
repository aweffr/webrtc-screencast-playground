...
公网部署可以使用 wildcard DNS、TLS certificate 和 Traefik HTTPS Ingress。示例 signaling server 部署到 `apps` namespace，通过普通 ClusterIP Service 暴露为 `wss://cast.example.com/ws`，并提供不含内部状态的 `/healthz`；server 和 client 同时支持无需 TLS 的 `ws://`。它不使用 coturn 的 `hostNetwork`、listener 或 relay port；WebSocket 与 TURN 保持独立故障域和运维入口。
...
- [Coturn project documentation](https://github.com/coturn/coturn/wiki)
