...
并以 release SHA-256 固定输入。CastTuning 配置迁移到 schema 2，Mac Sender 启用
Apple low-latency rate control；一期显式请求 Constrained Baseline 以优先兼容
Android emulator。若 SDP 与实际 SPS profile 不一致，沿用已确认的
`WARN_AND_CONTINUE` policy，在两端结构化记录 expected/actual profile 与 decoder
结果，不静默改写 SDP 或关闭 low latency。
