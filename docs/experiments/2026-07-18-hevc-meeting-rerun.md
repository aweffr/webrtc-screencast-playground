...
当前产品策略建议保持不变：默认仍为优先 H.264；`h265-only` 与 `prefer-h265` 保留为设备定向实验能力。随后单独补测的 B2（H.265，STATIC/ACTIVE MaxQP 27/36）没有取得可辨的静态文字收益，正文更新 p95 和峰值码率反而高于 B0。B0 仍是唯一值得进入下一轮 A/B 校准的 HEVC 参数，不再继续扩展 MaxQP 组合。
...
## 补充实验：B2（STATIC/ACTIVE MaxQP 27/36）

### 目的与有效性

B2 只回答一个收敛问题：在 B0 已经满足静态文字质量的前提下，进一步把 H.265 的 STATIC MaxQP 从 33 收紧到 27、同时把 ACTIVE MaxQP 从 39 收紧到 36，能否带来用户可见的文字清晰度收益，并且不损害正文更新延迟与码率。旧的 A0、A1、B0、B1 没有重跑；B2 独立重复三次，三次均在第一次 attempt 有效，0 invalid，0 retry。

补测仍使用同一份本地 Kubernetes 文档、Chrome `150.0.7871.129`、20 秒初始静止、六次 720 px 固定滚动和 20 秒最终静止。正式运行增加了原生全屏资格检查：真实主屏为 `1496×967 pt`，顶部安全区为 28 pt，三轮 Chrome 窗口均为 `1496×938 pt @ (0,29)`；页面 viewport 仍为 `1920×1080`。只有两套几何同时符合约束才开始媒体测量。因此本轮内容尺度、滚动位置和实际主屏占用均与旧正式样本一致，而不是把普通 Chrome 窗口误认为全屏。

在这项检查落地前曾启动过一次错误窗口状态的尝试，发现普通窗口后立即中止。该目录没有 `VALID` 标记，`attempts.jsonl` 也没有有效记录，未进入 B2 统计。正式补测的 evidence root 为 `artifacts/hevc-meeting-rerun-supplemental/20260718T133353Z/`，app commit 为 `a1537c0969c525a066ee429a2e04bb4a019c4266`。builder commit、WebRTC XCFramework/AAR 和 Chrome 版本与 base 批次相同；macOS executable SHA-256 为 `968778930d15fc925ee0dfe44e4da4950425b2f128f82c809ddb2b0ee68c3188`。

### 聚合结果与 B0 对照

| Case | STATIC/ACTIVE MaxQP | 实际 key/delta QP p95（最大） | 正文 E2E p50/p95 ms | render gap 范围 ms | VT drop | 峰值 bitrate Mbps | 最差 SSIM-Y / PSNR-Y | 人工文字检查 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| B0 | 33/39 | 33/35（37） | 173.8/233.4 | 537.1–614.5 | 0 | 1.98 | 0.996185 / 39.60 | 通过 |
| B2 | 27/36 | 33/31（36） | 190.2/253.9 | 577.1–628.3 | 0 | 2.30 | 0.996129 / 40.50 | 通过 |

B2 的 18 个正文滚动 sequence 全部送达，三轮都完成 6/6 个逐 burst 绑定的 ACTIVE→STATIC 往返。每次回到 STATIC 时，telemetry 都显示 MaxQP 27 已应用到当前 HEVC encoder session，对应关键帧 QP 为 27；进入 ACTIVE 后 MaxQP 36 也已应用。三轮合计没有 VideoToolbox drop，说明补测确实经过了目标配置路径。

与 B0 相比，B2 的最差 SSIM-Y 只变化 `-0.000056`，远小于 0.002 的预设容忍度；PSNR-Y 提高 0.90 dB，但人工原图中 12 px 中文、16 px 中英文、1 px 竖线、浅色边框和代码块没有可辨差异。这组结果不能证明把静态上限收紧到 27 带来了用户可见收益。

代价方向则不利于 B2：正文更新聚合 p95 从 233.4 ms 增至 253.9 ms，增加 20.6 ms（8.8%）；峰值 bitrate 从 1.98 Mbps 增至 2.30 Mbps，增加约 16.4%。三个 B2 run 各自的 p95 为 218.3–249.8 ms，与 B0 存在重叠，因此样本不足以把全部差异严格归因于 QP；但在没有可见画质收益的情况下，现有证据至少不支持用更高码率和更差的聚合延迟换取 B2。

B2 的首帧中位数为 2200.3 ms，不能与 B0 的 1326.1 ms 做参数归因。本轮为修复假全屏，将 Chrome 原生激活等待从 250 ms 增加到 1 秒，首帧指标又包含该启动阶段；跨批次差异混入了 750 ms 的实验时序变化。正文滚动发生在媒体已经稳定之后，不受这项启动等待影响，因此正文 E2E 仍可比较。

`max_render_gap` 仍沿用同一个已知会包含静态尾段的旧指标。B2 的 577.1–628.3 ms 没有改变“该指标目前不能证明真实 freeze”的判断，也不应被用来绕过原报告提出的 gap 测量修正。

下面第一张图检查 B2 自身的 sender→Android 一致性：左列为 sender capture，右列为 Android decoded；上排是初始静态位置，下排是六次滚动后的最终位置。

![B2 sender 与 Android 初始/最终对照](assets/2026-07-18-hevc-meeting-rerun/b2-sender-android-initial-final.png)

第二张图直接比较 Android decoded：左列为 B0，右列为 B2；上排是初始静态位置，下排是最终静态位置。两组的小字号文字、细线和浅色框没有可辨差异。

![B0 与 B2 Android 初始/最终对照](assets/2026-07-18-hevc-meeting-rerun/b0-b2-android-initial-final.png)

### 补充实验结论

B2 不替代 B0。把 H.265 STATIC/ACTIVE MaxQP 从 33/39 收紧到 27/36，虽然配置真实生效且没有引入丢帧，但没有产生用户可见的静态文字提升，正文更新延迟和码率方向反而更差。会议极速投屏的价值不是追求更低的 QP 数字，而是在文字已经清楚后保住交互延迟和带宽；因此仍保留 B0 33/39，停止继续向更低 HEVC MaxQP 扩展。
...
原始 evidence 包含构建日志、设备诊断、运行目录和网络过程信息，只保留在被 Git ignore 的本地 `artifacts/`，不进入版本库。提交的四张关键截图只包含固定 Kubernetes 文档 fixture 和实验 marker，不含浏览器账户、真实会议、联系人、邮箱、用户名、设备标识、凭据、私有 URL 或私有网络地址。图片已重新编码，并检查可见内容与嵌入文本；文件系统的 provenance 扩展属性不进入 Git blob。版本库只发布聚合结果、不可逆 artifact hash 和固定文档截图。

本报告只覆盖一台发送端、一台固定 Android TV 接收端、Chrome 150、production relay UDP 和单一文档型 workload。它足以淘汰 A1/B1/B2、确定 B0 为下一候选，并验证三项基础修复；尚不足以代表不同 Android 解码器、Wi-Fi 丢包、视频内容或长时间会议下的总体表现。
