...
- B0：首帧、ACTIVE p95、freeze、drop 和 bitrate 通过；content-aware 往返与静态图像质量失败。
...
本轮停止结论不依赖该限制：B0 即使忽略 ACTIVE p95，仍违反静态质量和 content-aware 门槛；A1/B1 还有独立的 freeze、质量或状态失败。
